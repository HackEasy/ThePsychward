...
puts "[*] C0d3d By Th3 M4D Ch3M15T & Th3 M4d Sc13nT15t\n </> M3nT4l Ho5p1T4l CoD3r5 </> "
...
puts "Coded By The M4d Sc13nT15t & M4d Ch3m15t".red
puts """
????????????????????????
????????????????????????
????????????????????????
...
?????????????????????
""".yellow
puts "Peeka-Boo, Were Starting To Poke You".red


print "\n[+] Enter Url(Example: ScriptKiddie.com): ".blue
url1 = gets.chomp
url2 = "http://" + url1 + "/"
print "\n[+] Enter Threads You Wish To Generate:".green
thread = gets.chomp
